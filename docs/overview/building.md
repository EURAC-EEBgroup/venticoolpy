...
- Construction mass; Cint (MJ/m³K): select predefined thermal mass of the construction. Heavy = 1.4 MJ/m³K, light = 0.9 MJ/m³K and medium = 1.15 MJ/m³K. NOTE: the tool gives the option to customize this value in the optional input session. In case a value different than .1 is entered for the construction mass in the optional input session, this input is ignored. 
